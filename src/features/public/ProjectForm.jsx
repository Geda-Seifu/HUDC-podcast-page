...
import { submitProject, uploadScreenshots } from "../../api/index.js";
...
const PROJECT_FIELDS = [
  {
    ...
    name: "title",
    label: "Project_Title",
    placeholder: "e.g. HUDC Dashboard",
    icon: Rocket,
    component: "input",
    type: "text",
  },
  {
    id: "field_project_author",
    name: "author",
    label: "Author_Name",
    placeholder: "e.g. Harry_root",
    icon: User,
    component: "input",
    type: "text",
  },
  {
    id: "field_project_github",
    name: "github_link",
  ...
  {
    id: "field_project_tech_stack",
    name: "tech_stack",
    label: "Environment_Stack",
  ...
  {
    id: "field_project_video",
    name: "video_demo_url",
    label: "Walkthrough_Video",
  ...
  {
    id: "field_project_twitter",
    name: "twitter_link",
    label: "X_Twitter_Link",
...
export default function ProjectForm({ isOpen }) {
  ...
  async function handleSubmit(e) {
    ...
    if (selectedFiles.length > 0) {
      screenshotUrls = await uploadScreenshots(selectedFiles);
    }
    ...
    const projectData = {
      ...
      tech_stack: rawData.tech_stack
        ? rawData.tech_stack.split(",").map((s) => s.trim())
        : [],
  ...
  if (isLoading)
    return (
      <div className="p-20 text-center font-mono text-[10px] animate-pulse">
        // SYNCING_SHOWCASE_CONFIG...
      </div>
    );
  if (!isOpen)
    return (
      <LockedState
        head={"Showcase Offline"}
        text={"// status: submissions_suspended_by_admin"}
      />
    );
    ...
    <div className="max-w-4xl mx-auto px-4 animate-in fade-in duration-700">
      <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between border-b border-hudc-light/20 pb-4 gap-4">
        <div>
          ...
          <p className="text-sm font-mono text-hudc-dark/60 mt-1">
      ...
      <div className="relative">
        ...
        <form
          ...
          className="bg-white p-6 md:p-10 rounded-sm border border-hudc-light/20 shadow-sm space-y-6"
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            {PROJECT_FIELDS.map((field) => {
              ...
              const InputTag =
                field.component === "textarea" ? "textarea" : "input";
          ...
          <div className="md:col-span-2 pt-4">
            ...
            <p className="text-center text-[9px] font-mono text-hudc-dark/40 mt-4 flex items-center justify-center gap-2 uppercase tracking-tighter">
              <Info className="w-3 h-3" /> // note: projects undergo manual
