...
export default function ProjectGallery() {
  const {
    ...
    queryFn: async ({ pageParam = 0 }) => {
      ...
      return { data, nextCursor: data.length === PROJECTS_PER_PAGE ? pageParam + 1 : undefined, total: count };
    },
    getNextPageParam: (lastPage) => lastPage.nextCursor,
    initialPageParam: 0,
  });

  if (isLoading) return <div className="text-center py-20 font-mono text-hudc-blue animate-pulse uppercase tracking-widest">{'>'} booting_gallery...</div>;
  if (isError) return <div className="text-center py-20 text-red-500 font-mono italic">Error: sync_failed. Check console.</div>;
            ...
            <div className="p-6 flex flex-col grow">
              ...
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-1">
                  <a href={project.github_link} target="_blank" rel="noreferrer" className="p-1.5 text-hudc-dark/40 hover:text-hudc-blue transition-colors border border-transparent hover:border-hudc-light/30 rounded-[2px]">
                    <Github className="w-4 h-4" />
                  </a>
                  {project.demo_link && (
                    <a href={project.demo_link} target="_blank" rel="noreferrer" className="p-1.5 text-hudc-dark/40 hover:text-hudc-blue transition-colors">
      ...
      {hasNextPage && (
        <div className="mt-16 flex flex-col items-center gap-4">
          <button 
            ...
            className="flex items-center gap-3 px-8 py-3 bg-white border border-hudc-light/30 text-hudc-dark font-mono text-xs font-bold rounded-[4px] hover:bg-hudc-bg transition-all border-b-4 border-black/10 active:border-b-0 active:translate-y-[2px] disabled:opacity-50"
          >
